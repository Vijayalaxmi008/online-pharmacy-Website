...
const Register = () => {
  ...
  const handleNext = (e) => {
    ...
    if (phoneClean.length !== 11) { setError('Введите телефон в формате +7XXXXXXXXXX'); return }
  ...
  const submit = (e) => {
    ...
    if (!form.agree) { setError('Необходимо согласие'); return }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-white flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
        ...
        <Link to="/login" className="text-primary-500 text-sm mb-3 inline-block">Войти</Link>
        ...
        {step === 1 && (
          <form onSubmit={handleNext} className="space-y-3">
            ...
            <div className="relative">
              ...
              <input required type={show ? 'text' : 'password'} placeholder="Пароль" value={form.password} onChange={e => setForm({...form, password: e.target.value})}
            ...
            <button type="submit" className="w-full bg-primary-500 text-white font-bold py-2.5 rounded-full text-sm">ДАЛЕЕ</button>
          </form>
        )}
        {step === 2 && (
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="relative">
              <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input required type="password" placeholder="Повторите пароль" value={form.confirmPassword} onChange={e => setForm({...form, confirmPassword: e.target.value})}
                className="w-full pl-9 pr-3 py-2.5 border-2 border-gray-200 rounded-full text-sm focus:border-primary-500 focus:outline-none" />
            </div>
            ...
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-primary-500 text-white font-bold py-2.5 rounded-full text-sm">РЕГИСТРАЦИЯ</button>
              <button type="button" onClick={() => setStep(1)} className="text-gray-500 text-sm px-3">Назад</button>
            </div>
          </form>
        )}
      </div>
