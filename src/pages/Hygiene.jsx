...
import {
  ChevronRight, ChevronLeft, ChevronDown, ListFilter, PackageSearch,
  Droplet, Pill, SprayCan, Heart, Minus, Plus, X, Star,
  ThumbsUp, ThumbsDown, ShoppingCart, MapPin, Truck,
} from 'lucide-react'
...
const hygieneProducts = [
  {
    ...
    country: 'Germany', effect: 'For teeth',
  ...
  },
  {
    ...
    category: 'hygiene', code: 153255, packQty: 10,
    country: 'Italy', effect: 'For hands',
  ...
  },
  {
    ...
    category: 'hygiene', code: 153256, packQty: 10,
    country: 'Italy', effect: 'For hands',
  ...
  },
  {
    ...
    country: 'Germany', effect: 'For hair',
  ...
  },
  {
    ...
    country: 'Ireland', effect: 'For hands',
  },
  {
    ...
    category: 'hygiene', code: 153257, packQty: 10,
    country: 'Spain', effect: 'For body',
  ...
  },
  {
    ...
    category: 'hygiene', code: 153258, packQty: 10,
    country: 'Germany', effect: 'For body',
  ...
  },
  {
    ...
    category: 'hygiene', code: 153259, packQty: 10,
    country: 'Italy', effect: 'For ears',
  ...
  },
  {
    ...
    country: 'China', effect: 'For teeth',
  ...
  },
  {
    ...
    country: 'Greece', effect: 'For teeth',
  },
]

const catalogTiles = [
  { name: 'Oral care', icon: Droplet },
  { name: 'Hand & body wash', icon: SprayCan },
  { name: 'Deodorants', icon: Pill },
  { name: 'Skin care', icon: Droplet },
  { name: 'Feminine hygiene', icon: Pill },
  { name: 'Shaving & hair removal', icon: Droplet },
  { name: 'Wipes & sanitizers', icon: Droplet },
  { name: 'Cotton & swabs', icon: SprayCan },
]

const popularBrands = [
  'DentaFresh', 'CleanHands', 'HairCare', 'FreshDay', 'SmoothShave',
  'NeviaCare', 'Oral-B', 'Nivea', 'Dove', 'Colgate',
  'Head & Shoulders', 'Nike', 'Vichy', 'La Roche-Posay', 'Bioderma',
  'Sebamed', 'Avene', 'Uriage', 'Eucerin', 'CeraVe',
  'Neutrogena', 'Garnier', 'Palmolive', "Johnson's", 'Listerine',
  'Oral-B', 'Nivea', 'Dove',
]

const countries = ['Belarus', 'Germany', 'Greece', 'Ireland', 'Spain', 'Italy', 'China']
const effects = ['For teeth', 'For hands', 'For hair', 'For body', 'For ears', 'For face']
...
const Hygiene = () => {
  ...
  useEffect(() => {
    if (selectedProduct) {
      setActiveTab('main'); setInfoTab('delivery'); setActiveTopic(INSTRUCTION_TOPICS[0])
      setInstructionsExpanded(false); setShowAllReviews(false); setQty(1); setGalleryIndex(0)
  ...
  const relatedProducts = selectedProduct
    ? filtered.filter((p) => p.id !== selectedProduct.id).slice(0, 5)
    : []
  const isFav = selectedProduct ? favorites.includes(selectedProduct.id) : false
  const gallery = selectedProduct ? [selectedProduct.image, selectedProduct.image, selectedProduct.image, selectedProduct.image] : []
  ...
  return (
    <div className="bg-gray-100">
      <div className="container mx-auto px-4 py-6">
        ...
        {selectedProduct ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6">
            <div className="flex items-center justify-between gap-3 mb-2">
              <button onClick={() => setSelectedProduct(null)} className="flex items-center gap-1 text-xs font-medium text-gray-500 hover:text-primary-500">
                <ChevronLeft size={14} /> Back to list
              </button>
              <button onClick={() => setSelectedProduct(null)} className="w-8 h-8 shrink-0 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400" aria-label="Close product details">
                <X size={16} />
              </button>
            </div>

            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4">{selectedProduct.name}</h1>
            ...
            <div ref={sectionRefs.main} className="grid lg:grid-cols-[64px_1fr_280px_260px] gap-4 mb-10 scroll-mt-20">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-6 relative flex items-center justify-center min-h-[260px]">
                <div className="absolute top-4 left-4 flex flex-col gap-1.5">
                  {selectedProduct.isProductOfDay && (
                ...
                <img src={gallery[galleryIndex]} alt={selectedProduct.name} className="w-full max-w-[220px] aspect-square object-contain" />
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className={`ml-auto font-medium ${selectedProduct.inStock ? 'text-primary-500' : 'text-red-500'}`}>
                    {selectedProduct.inStock ? 'In stock' : 'Out of stock'}
                  </span>
                </div>
                <div className="text-xs text-gray-400">Item code: {selectedProduct.code}</div>
                ...
                <div className="border border-gray-100 rounded-xl p-4">
                  ...
                  <ul className="text-xs space-y-1.5 text-gray-600">
                    <li>• Manufacturer: <span className="text-primary-500">{selectedProduct.brand}</span></li>
                    <li>• Active substance: {selectedProduct.brand}</li>
                    <li>• Shelf life: Long shelf life</li>
                    <li>• Producer: <span className="text-primary-500">{selectedProduct.brand}</span></li>
                  </ul>
                </div>
              ...
              <div className="border border-gray-100 rounded-2xl p-5 h-fit">
                ...
                {selectedProduct.oldPrice && (
                  <div className="text-sm text-red-400 line-through">{selectedProduct.oldPrice.toLocaleString('en-US')} rub.</div>
                )}
                <div className="text-2xl font-bold text-gray-900 mb-4">{selectedProduct.price.toLocaleString('en-US')} rub.</div>
            ...
            {analogues.length > 0 && (
              <div className="mb-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
                  {analogues.map((p) => <ProductCard key={`analog-${p.id}`} product={p} onSelect={setSelectedProduct} />)}
                </div>
              </div>
            )}
            ...
            {analogues.length > 0 && (
              <div className="mb-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {analogues.map((p) => <ProductCard key={`ag-${p.id}`} product={p} onSelect={setSelectedProduct} />)}
                </div>
              </div>
            )}
          ...
          <>
            ...
            <div className="flex gap-4">
              <aside className="hidden lg:block w-64 shrink-0 space-y-4">
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm">
                  <h3 className="font-bold text-xs tracking-wide text-gray-400 mb-3">Country</h3>
                ...
                <div className="bg-white rounded-2xl p-4 shadow-sm">
                  <h3 className="font-bold text-xs tracking-wide text-gray-400 mb-3">Effect</h3>
                  <input
                    type="text"
                    placeholder="Search..."
                    value={effectSearch}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => { setMaxPrice(50000); setInStockOnly(false); setSelectedCountries([]); setSelectedEffects([]); setEffectSearch('') }}
              ...
              <div className="flex-1 min-w-0">
                ...
                <section className="mt-10">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex gap-2">
                      <button className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center hover:bg-white text-gray-500"><ChevronLeft size={14} /></button>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                    {hygieneProducts.slice(0, 4).map((p) => <ProductCard key={`deal-${p.id}`} product={p} onSelect={setSelectedProduct} />)}
                  </div>
                </section>
                ...
                <section className="mt-10">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                    {hygieneProducts.slice(3, 7).map((p) => <ProductCard key={`viewed-${p.id}`} product={p} onSelect={setSelectedProduct} />)}
                  </div>
                </section>
              </div>
            </div>
        ...
        {/* SEO text — always shows, on both the listing view and the product detail view */}
        <section className="mt-10 bg-white rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-bold text-gray-900 mb-3">SEO text</h2>
          <div className="text-xs text-gray-500 leading-relaxed grid md:grid-cols-2 gap-6">
            <p>
              Buy personal care and hygiene products in Moscow and the surrounding region with home delivery
              through our online pharmacy. We offer a wide range of oral care, hand and body wash, and
              feminine hygiene products from trusted manufacturers, affordable prices, and fast delivery
              throughout Russia.
            </p>
            <p>
              We guarantee the authenticity of all our products — every item is certified for quality and
              compliance. Place your order online or collect it from the nearest pharmacy in our partner network.
            </p>
          </div>
        </section>
      </div>
