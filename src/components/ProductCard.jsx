    ...
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition overflow-hidden flex flex-col relative">
      ...
      <div className="relative p-3">
        {product.isProductOfDay && (
          <span className="absolute top-3 left-3 bg-primary-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full z-10">Товар дня</span>
        )}
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <ul className="text-[10px] text-gray-500 space-y-0.5 mb-2">
          <li>· Бренд: <span className="text-primary-500">{product.brand}</span></li>
          <li>· Количество в упаковке: 10 шт</li>
          <li>· Код товара: {product.code}</li>
        </ul>
        <div className="mt-auto">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-lg font-bold">{product.price.toLocaleString('ru-RU')} РУБ.</span>
          ...
          {product.oldPrice && <div className="text-xs text-red-500 line-through mb-2">{product.oldPrice.toLocaleString('ru-RU')} РУБ.</div>}
          {product.inStock ? (
            <button onClick={() => addToCart(product)} className="w-full bg-primary-500 text-white font-bold py-2.5 rounded-xl text-xs hover:bg-primary-600">Купить в 1 клик</button>
          ) : (
            <button className="w-full bg-gray-300 text-gray-600 font-bold py-2.5 rounded-xl text-xs">Сообщить о наличии</button>
          )}
        </div>
      </div>
    </div>
