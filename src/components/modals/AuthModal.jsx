  ...
  return (
    <Modal onClose={onClose}>
      <div className="p-5">
        ...
        <p className="text-xs text-gray-600 mb-4">User not found User not found Invalid password.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === 'login' ? (
            ...
            <>
              <div className="relative">
                ...
                <input required type="tel" placeholder="Your phone" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})}
                  className="w-full pl-10 pr-3 py-2.5 border-2 border-gray-200 rounded-full focus:border-primary-500 focus:outline-none text-sm" />
              </div>
              <div className="relative">
                <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          ...
          <div className="flex items-center gap-3 pt-1">
            <button type="submit" className="bg-primary-500 text-white font-bold px-6 py-2.5 rounded-full hover:bg-primary-600 text-xs">{mode === 'login' ? 'SIGN IN' : 'REGISTER'}</button>
            <p className="text-[10px] text-gray-500 flex-1">By clicking the button, you agree to the processing of <span className="text-primary-500">personal data</span></p>
          </div>
        </form>
      </div>
    </Modal>
